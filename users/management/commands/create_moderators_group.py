...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS('Права для группы "Moderators" настроены')
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Ошибка при настройке прав группы "Moderators": {e}')
            )
        ...
        for moderator_data in moderators_data:
            ...
            user, created = User.objects.get_or_create(
                email=email,
                defaults={"is_staff": moderator_data["is_staff"], "is_active": True},
            )
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Пользователь уже существует: {email}")
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Пользователь {email} добавлен в группу модераторов"
                )
            )
