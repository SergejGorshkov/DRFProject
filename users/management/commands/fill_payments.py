...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        course, created = Course.objects.get_or_create(
            name="Курс 1", description="Какой-то курс1"
        )
        if created:
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.WARNING(f'Курс "{course.name}" уже есть в БД.')
            )
        ...
        for lesson_data in lessons:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Успешно добавлен урок: {lesson.name}")
                )
                created_lessons.append(lesson)
            else:
                self.stdout.write(
                    self.style.WARNING(f'Урок "{lesson.name}" уже есть в БД.')
                )
        ...
        for user_data in users:
            ...
            if created:
                ...
                user.save()
                self.stdout.write(
                    self.style.SUCCESS(f"Успешно создан пользователь: {user.email}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'Пользователь "{user.email}" уже есть в БД.')
                )
        ...
        for payment_data in payments:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Успешно добавлен платеж: {payment.user.email} - {payment.amount} руб."
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f'Платеж для "{payment.user.email}" уже есть в БД.'
                    )
                )
